export const ru = {
  common: {
    cancel: "\u041e\u0442\u043c\u0435\u043d\u0430",
    edit: "\u0420\u0435\u0434\u0430\u043a\u0442\u0438\u0440\u043e\u0432\u0430\u0442\u044c",
    save: "\u0421\u043e\u0445\u0440\u0430\u043d\u0438\u0442\u044c",
    delete: "\u0423\u0434\u0430\u043b\u0438\u0442\u044c",
    add: "\u0414\u043e\u0431\u0430\u0432\u0438\u0442\u044c",
    account: "\u0410\u043a\u043a\u0430\u0443\u043d\u0442",
    logout: "\u0412\u044b\u0439\u0442\u0438",
    themeToggle: "\u0422\u0435\u043c\u0430",
    errorOccurred:
      "\u041f\u0440\u043e\u0438\u0437\u043e\u0448\u043b\u0430 \u043e\u0448\u0438\u0431\u043a\u0430.",
    all: "\u0412\u0441\u0435",
    reset: "\u0421\u0431\u0440\u043e\u0441\u0438\u0442\u044c",
    search: "\u041f\u043e\u0438\u0441\u043a",
  },
  dashboard: {
    empty:
      "\u041d\u0438\u0447\u0435\u0433\u043e \u043d\u0435 \u0432\u044b\u0431\u0440\u0430\u043d\u043e",
    selectPrompt:
      "\u0412\u044b\u0431\u0435\u0440\u0438\u0442\u0435 \u043e\u0431\u044a\u0435\u043a\u0442...",
    add: "\u0414\u043e\u0431\u0430\u0432\u0438\u0442\u044c",
    import: "\u0418\u043c\u043f\u043e\u0440\u0442",
    export: "\u042d\u043a\u0441\u043f\u043e\u0440\u0442",
  },
  env: {
    missingVarsPrefix:
      "\u041e\u0442\u0441\u0443\u0442\u0441\u0442\u0432\u0443\u044e\u0442 \u043f\u0435\u0440\u0435\u043c\u0435\u043d\u043d\u044b\u0435 \u043e\u043a\u0440\u0443\u0436\u0435\u043d\u0438\u044f:",
    limitedMode:
      "\u041f\u0440\u0438\u043b\u043e\u0436\u0435\u043d\u0438\u0435 \u0440\u0430\u0431\u043e\u0442\u0430\u0435\u0442 \u0432 \u043e\u0433\u0440\u0430\u043d\u0438\u0447\u0435\u043d\u043d\u043e\u043c \u0440\u0435\u0436\u0438\u043c\u0435.",
    apiUnavailable:
      "API \u043d\u0435\u0434\u043e\u0441\u0442\u0443\u043f\u043d\u043e: \u043f\u0440\u043e\u0432\u0435\u0440\u044c\u0442\u0435 VITE_API_BASE_URL.",
    targetsPrefix: "\u0414\u043b\u044f: ",
  },
  objects: {
    edit: "\u0420\u0435\u0434\u0430\u043a\u0442\u0438\u0440\u043e\u0432\u0430\u0442\u044c \u043e\u0431\u044a\u0435\u043a\u0442",
    delete:
      "\u0423\u0434\u0430\u043b\u0438\u0442\u044c \u043e\u0431\u044a\u0435\u043a\u0442",
    empty:
      "\u041d\u0435\u0442 \u043e\u0431\u044a\u0435\u043a\u0442\u043e\u0432",
  },
  inventory: {
    tabs: {
      desc: "\u041e\u043f\u0438\u0441\u0430\u043d\u0438\u0435",
      hw: "\u0416\u0435\u043b\u0435\u0437\u043e",
      tasks: "\u0417\u0430\u0434\u0430\u0447\u0438",
      chat: "\u0427\u0430\u0442",
    },
    noDescription:
      "\u041d\u0435\u0442 \u043e\u043f\u0438\u0441\u0430\u043d\u0438\u044f",
  },
  hardware: {
    header:
      "\u041e\u0431\u043e\u0440\u0443\u0434\u043e\u0432\u0430\u043d\u0438\u0435",
    addTitle:
      "\u0414\u043e\u0431\u0430\u0432\u0438\u0442\u044c \u043e\u0431\u043e\u0440\u0443\u0434\u043e\u0432\u0430\u043d\u0438\u0435",
    editTitle:
      "\u0418\u0437\u043c\u0435\u043d\u0438\u0442\u044c \u043e\u0431\u043e\u0440\u0443\u0434\u043e\u0432\u0430\u043d\u0438\u0435",
    notFound:
      "\u041e\u0431\u043e\u0440\u0443\u0434\u043e\u0432\u0430\u043d\u0438\u0435 \u043d\u0435 \u043d\u0430\u0439\u0434\u0435\u043d\u043e",
    name: "\u041d\u0430\u0437\u0432\u0430\u043d\u0438\u0435",
    location:
      "\u0420\u0430\u0441\u043f\u043e\u043b\u043e\u0436\u0435\u043d\u0438\u0435",
    purchaseStatus:
      "\u0421\u0442\u0430\u0442\u0443\u0441 \u043f\u043e\u043a\u0443\u043f\u043a\u0438",
    installStatus:
      "\u0421\u0442\u0430\u0442\u0443\u0441 \u0443\u0441\u0442\u0430\u043d\u043e\u0432\u043a\u0438",
    choosePurchaseStatus:
      "\u0412\u044b\u0431\u0435\u0440\u0438\u0442\u0435 \u0441\u0442\u0430\u0442\u0443\u0441 \u043f\u043e\u043a\u0443\u043f\u043a\u0438",
    chooseInstallStatus:
      "\u0412\u044b\u0431\u0435\u0440\u0438\u0442\u0435 \u0441\u0442\u0430\u0442\u0443\u0441 \u0443\u0441\u0442\u0430\u043d\u043e\u0432\u043a\u0438",
    statusPurchasePrefix:
      "\u0421\u0442\u0430\u0442\u0443\u0441 \u043f\u043e\u043a\u0443\u043f\u043a\u0438:",
    statusInstallPrefix:
      "\u0421\u0442\u0430\u0442\u0443\u0441 \u0443\u0441\u0442\u0430\u043d\u043e\u0432\u043a\u0438:",
    statuses: {
      purchase: {
        not_paid: "\u043d\u0435 \u043e\u043f\u043b\u0430\u0447\u0435\u043d",
        paid: "\u043e\u043f\u043b\u0430\u0447\u0435\u043d",
        unknown: "\u043d\u0435 \u0443\u043a\u0430\u0437\u0430\u043d",
      },
      install: {
        not_installed:
          "\u043d\u0435 \u0443\u0441\u0442\u0430\u043d\u043e\u0432\u043b\u0435\u043d",
        installed:
          "\u0443\u0441\u0442\u0430\u043d\u043e\u0432\u043b\u0435\u043d",
        unknown: "\u043d\u0435 \u0443\u043a\u0430\u0437\u0430\u043d",
      },
    },
    validation: {
      nameRequired:
        "\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u043d\u0430\u0437\u0432\u0430\u043d\u0438\u0435",
      purchaseStatusRequired:
        "\u0412\u044b\u0431\u0435\u0440\u0438\u0442\u0435 \u0441\u0442\u0430\u0442\u0443\u0441 \u043f\u043e\u043a\u0443\u043f\u043a\u0438",
      installStatusRequired:
        "\u0412\u044b\u0431\u0435\u0440\u0438\u0442\u0435 \u0441\u0442\u0430\u0442\u0443\u0441 \u0443\u0441\u0442\u0430\u043d\u043e\u0432\u043a\u0438",
    },
    errors: {
      load: "\u041e\u0448\u0438\u0431\u043a\u0430 \u0437\u0430\u0433\u0440\u0443\u0437\u043a\u0438 \u043e\u0431\u043e\u0440\u0443\u0434\u043e\u0432\u0430\u043d\u0438\u044f",
      create:
        "\u041e\u0448\u0438\u0431\u043a\u0430 \u0434\u043e\u0431\u0430\u0432\u043b\u0435\u043d\u0438\u044f \u043e\u0431\u043e\u0440\u0443\u0434\u043e\u0432\u0430\u043d\u0438\u044f",
      update:
        "\u041e\u0448\u0438\u0431\u043a\u0430 \u043e\u0431\u043d\u043e\u0432\u043b\u0435\u043d\u0438\u044f \u043e\u0431\u043e\u0440\u0443\u0434\u043e\u0432\u0430\u043d\u0438\u044f",
      delete:
        "\u041e\u0448\u0438\u0431\u043a\u0430 \u0443\u0434\u0430\u043b\u0435\u043d\u0438\u044f \u043e\u0431\u043e\u0440\u0443\u0434\u043e\u0432\u0430\u043d\u0438\u044f",
    },
  },
  tasks: {
    statuses: {
      planned: "Запланирована",
      in_progress: "В работе",
      done: "Выполнена",
      canceled: "Отменена",
    },
    viewModeLabel: "Режим представления",
    viewModes: {
      list: "Список",
      kanban: "Канбан",
      analytics: "Аналитика",
    },
    headerPrefix: "Задачи для",
    add: "Добавить задачу",
    empty: "Задач пока нет. Добавьте первую задачу.",
    editTitle: "Редактировать задачу",
    addTitle: "Добавить задачу",
    title: "Название",
    assignee: "Исполнитель",
    assigneePlaceholder: "Имя или email",
    searchPlaceholder: "Название, исполнитель, дата (дд.мм.гггг)",
    status: "Статус",
    chooseStatus: "Выберите статус",
    notes: "Заметки",
    confirmDelete: "Удалить задачу?",
    form: {
      dueDate: "Назначена на",
    },
    view: {
      assignee: "Исполнитель:",
      added: "Добавлена:",
      dueDate: "Назначена на:",
      status: "Статус:",
      notes: "Заметки:",
    },
    kanban: {
      empty: "Нет задач для отображения.",
      columnEmpty: "Нет задач",
    },
    analytics: {
      title: "Аналитика задач",
      empty: "Недостаточно данных для построения аналитики.",
      total: "Всего задач",
      completed: "Завершено",
      overdue: "Просрочено",
      byStatus: "Распределение по статусам",
    },
  },
  auth: {
    register:
      "\u0420\u0435\u0433\u0438\u0441\u0442\u0440\u0430\u0446\u0438\u044f",
    login: "\u0412\u043e\u0439\u0442\u0438",
    email: "Email",
    username:
      "\u0418\u043c\u044f \u043f\u043e\u043b\u044c\u0437\u043e\u0432\u0430\u0442\u0435\u043b\u044f",
    password: "\u041f\u0430\u0440\u043e\u043b\u044c",
    already:
      "\u0423\u0436\u0435 \u0435\u0441\u0442\u044c \u0430\u043a\u043a\u0430\u0443\u043d\u0442? \u0412\u043e\u0439\u0442\u0438",
    noAccount:
      "\u041d\u0435\u0442 \u0430\u043a\u043a\u0430\u0443\u043d\u0442\u0430? \u0417\u0430\u0440\u0435\u0433\u0438\u0441\u0442\u0440\u0438\u0440\u043e\u0432\u0430\u0442\u044c\u0441\u044f",
    checkEmail:
      "\u041f\u0440\u043e\u0432\u0435\u0440\u044c\u0442\u0435 \u044d\u043b\u0435\u043a\u0442\u0440\u043e\u043d\u043d\u0443\u044e \u043f\u043e\u0447\u0442\u0443 \u0434\u043b\u044f \u043f\u043e\u0434\u0442\u0432\u0435\u0440\u0436\u0434\u0435\u043d\u0438\u044f",
    networkError:
      "\u041f\u0440\u043e\u0431\u043b\u0435\u043c\u0430 \u0441 \u0441\u0435\u0442\u044c\u044e. \u041f\u043e\u043f\u0440\u043e\u0431\u0443\u0439\u0442\u0435 \u043f\u043e\u0437\u0436\u0435.",
  },
};
